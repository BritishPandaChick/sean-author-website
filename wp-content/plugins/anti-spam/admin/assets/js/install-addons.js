    ...
    var wtitan_externalAddon = {
        ...
        events: function () {
            ...
            $(document).on('click', '.wtitan-update-component-button', function () {
                ...
                $.wbcr_factory_clearfy_223.hooks.run('clearfy/components/pre_update', [$this, data]);

                self.sendRequest(data, function (response) {
                    if (!response || !response.success) {

                        if (response.data && response.data.error_message) {
                            $.wbcr_factory_clearfy_223.app.showNotice(response.data.error_message, 'danger');
                        }

                        $.wbcr_factory_clearfy_223.hooks.run('clearfy/components/update_error', [
                            $this,
                            data,
                    ...
                    if (response.success) {
                        ...
                        if (storage == 'freemius') {
                            if (response.data.update_notice) {
                                if (!$('.wtitan-update-package').length) {
                                    $.wbcr_factory_clearfy_223.app.showNotice(response.data.update_notice);
                                }
                            } else {
                                if ($('.wtitan-update-package').length) {
                        ...
                        if (plugin_action == 'install') {
                            ...
                            $.wbcr_factory_clearfy_223.hooks.run('clearfy/components/installed', [
                                $this,
                                data,
                                response
                            ]);
                            ...
                            $.wbcr_factory_clearfy_223.hooks.run('clearfy/components/pre_activate', [
                                $this,
                                data,
                                response
                            ]);
                            ...
                            $.wbcr_factory_clearfy_223.hooks.run('clearfy/components/deactivated', [
                                $this,
                                data,
                                response
                            ]);
                            ...
                            $.wbcr_factory_clearfy_223.hooks.run('clearfy/components/deleted', [$this, data, response]);
                        }
                    } else {
                        if (plugin_action == 'install') {
                    ...
                    if (response.data.need_rewrite_rules && !$('.wtitan-need-rewrite-rules-message').length) {
                        $.wbcr_factory_clearfy_223.app.showNotice(response.data.need_rewrite_rules, 'warning');
                    }

                    $.wbcr_factory_clearfy_223.hooks.run('clearfy/components/updated', [$this, data, response]);
                });
        ...
        sendRequestToComponentActivationPrepare: function (componentButton, sendData, button_i18n) {
            ...
            this.sendRequest(sendData, function (response) {
                ...
                if (!response || !response.success) {
                    ...
                    $.wbcr_factory_clearfy_223.hooks.run('clearfy/components/activated_error', [sendData.plugin]);
                    return;
                }
                ...
                $.wbcr_factory_clearfy_223.hooks.run('clearfy/components/activated', [sendData.plugin]);
            });
        },
        ...
        sendRequest: function (data, callback) {
            ...
            $.ajax(ajaxurl, {
                ...
                error: function (xhr, ajaxOptions, thrownError) {
                    ...
                    $.wbcr_factory_clearfy_223.app.showNotice('Error: [' + thrownError + '] Status: [' + xhr.status + '] Error massage: [' + xhr.responseText + ']', 'danger');
                }
            });
        }
    };
